...
if "%menu_choice%"=="all" goto SERVICE_ALL
...
if "%menu_choice%"=="all" goto LOG_ALL
...
if "%menu_choice%"=="all" goto SECURITY_ALL
...
if not errorlevel 1 (
    ...
    ECHO 설명 : " 컴퓨터에 대한 원격 연결을 허용하고 있어 불필요한 계정이 있는지 수동 진단을 권장합니다." >> check.txt
    ECHO [보안 조치] >> check.txt
    ECHO Step 1: 제어판 ^> 사용자 계정 ^> 관리자 계정 이외의 계정 생성 >> check.txt
    ECHO Step 2: 제어판 ^> 시스템 ^> '원격' 탭 ^> '원격' 탭 메뉴에서 "사용자가 이 컴퓨터에 원격으로 연결할 수 있음"에 체크 > "원격 사용자 선택"에서 원격 사용자 지정후 확인 >> check.txt
    ECHO --------------------------------------------------------------------------------- >> check.txt
    ECHO. >> check.txt
    ECHO. >> check.txt
    ECHO ---------------------------------------------------------------------------------
...
if %vulnerable%==0 (
    ...
    for %%S in (%services%) do (
        net start | findstr /C:%%S >nul
        if !errorlevel!==0 ( 
            echo %%S 서비스가 실행 중이기 때문에 취약합니다. >> bad.txt
        )
    )
    ECHO. >> bad.txt
